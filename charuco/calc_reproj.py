...
cam_image_file = "cap/image%03d.png"
...
param_folder = "ceres-calibration/ceres-calibration/build/estimated"

cam_d = np.loadtxt(param_folder+"/cam_d.txt",delimiter=",")
cam_k = np.loadtxt(param_folder+"/cam_k.txt",delimiter=",")
cam_r = np.loadtxt(param_folder+"/cam_r.txt",delimiter=",")
cam_t = np.loadtxt(param_folder+"/cam_t.txt",delimiter=",")

cam_r = np.reshape(cam_r,[valid_id_num,3,1])
cam_t = np.reshape(cam_t,[valid_id_num,3,1])
...
for i in range(len(cam_3d_point_list)):
    ...
    image = cv2.imread(cam_image_file%image_id,1)
